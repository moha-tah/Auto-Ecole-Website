    ...
    <!-- Page de droite -->
	    ...
	    <?php
	        ...
	        require("data_bdd.php");
					...
					echo "<h1>Notation d'une séance</h1>";

	        echo '<h2>Quelle séance voulez-vous noter ?</h2><br>';

					// on récupère toutes les séances dans le passé.
	        $requeteSeancesPassees = 'SELECT s.idseance, s.dateSeance, t.nom FROM seances AS s INNER JOIN themes AS t
	                  ON s.dateSeance<CURDATE() AND s.idtheme = t.idtheme
										AND (SELECT COUNT(i.ideleve) FROM inscription as i WHERE i.idseance = s.idseance) > 0
										ORDER BY s.dateSeance';
	        $listeSeancesPassees = mysqli_query($connect, $requeteSeancesPassees);

	        echo '<form method="POST" action="valider_seance.php">';

	        echo 'Séance : <select name="choixSeance" required>';
	        echo '<option value="" selected disabled hidden> - choisissez une séance - </option>';

					// on ajoute chaque séance en option du select.
	        while ($seance = mysqli_fetch_array($listeSeancesPassees, MYSQLI_NUM)) {
							$idseance = $seance[0];
							$date = $seance[1];
							$nomTheme = $seance[2];
